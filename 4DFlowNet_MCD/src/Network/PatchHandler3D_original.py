...
class PatchHandler3D():
    ...
    def load_patches_from_index(self, index_row):
        ...
        u_patch, u_hr_patch, v_patch, v_hr_patch, w_patch, w_hr_patch, mag_patch, venc, mask_patch = self.load_vectorfield(hd5path, lr_hd5path, idx, mask_index, patch_index, hr_patch_index)
        ...
        return (
            ...
            mag_patch[..., np.newaxis].astype(np.float32),
            np.array([venc], dtype=np.float32),
            mask_patch.astype(np.float32),
            np.array([invivo_flag], dtype=np.float32), #!#
        )
    ...
    def load_patches_from_index_file(self, indexes):
        ...
        if (u_patch.shape, v_patch.shape, w_patch.shape) != (size, size, size):
            print("At (x_start, y_start, z_start) =", (x_start, y_start, z_start))
            print("Shapes after rotation:", u_patch.shape, v_patch.shape, w_patch.shape, mask_patch.shape)
            print((self.patch_size, self.patch_size, self.patch_size))
