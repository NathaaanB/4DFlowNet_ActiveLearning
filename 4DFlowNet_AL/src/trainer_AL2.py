...
if __name__ == "__main__":
    ...
    benchmark_file = f'{data_dir}/test16.csv'
    training_file = f'{data_dir}/train.csv'
    validate_file = f'{data_dir}/validate.csv'

    keys = ["aorta01_LR.h5", "aorta02_LR.h5"]
    keys = ["example_data.h5"]
    ...
    random_state = 42
    uncertainty_metric = "var_mean"  # "var_mean", "var_max"
    AL_loops = 5
    ...
    for k in range(AL_loops):
        ...
        df = pd.DataFrame(retrain_indexes, columns=["source", "target", "index", "start_x", "start_y", "start_z", "rotate", "rotation_plane", "rotation_degree_idx", "coverage"])
        ...
        df_most_uncertain.to_csv(f"../models/networks_trained_{timestamp_folder}/most_uncertain_AL{k+1}.csv", index=False)
        df_remaining.to_csv(f"../models/{folder_name}/remaining_patches_AL{k+1}.csv", index=False)
        for i, key in enumerate(keys):
            html_uncertain_patches(f"../models/networks_trained_{timestamp_folder}/most_uncertain_AL{k+1}.csv", f"../models/networks_trained_{timestamp_folder}/selected_patches_key0{i+1}_AL{k+1}.html", f"../../data/{key}", key)
        ...
        print("top_patches shape:", top_patches.shape)
