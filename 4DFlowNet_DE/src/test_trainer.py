...
if __name__ == "__main__":
    ...
    train_indexes_woAL = train_indexes[:len(train_indexes)//2]
    train_indexes_wAL = train_indexes[len(train_indexes)//2:]
    ...
    for model_id in range(n_models):
        ...
        "epochs_wo_AL": epochs_wo_AL,
        "epochs_w_AL": epochs_w_AL,
        ...
        trainset = ph_train.initialize_dataset_preloaded(train_indexes_woAL, shuffle=True)

        ph_train_AL = PatchHandler3D(data_dir, patch_size, res_increase, batch_size, rotation, mask_threshold)
        trainset = ph_train_AL.initialize_dataset_preloaded(train_indexes_wAL, shuffle=True)
        ...
        network.train_network(trainset, valset, n_epoch_wo_AL=epochs_wo_AL, n_epochs_w_AL = epochs_w_AL, testset=testset)
